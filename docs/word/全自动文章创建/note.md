...
if (args.length < 1) {
    console.error('请提供两个或者一个参数：代码块标题 和要删除的文件夹名称');
    process.exit(1);
}
...
if (!fs.existsSync(docsPath)) {
    console.error(`错误：docs 文件夹不存在: ${docsPath}`);
    process.exit(1);
}

if (!fs.existsSync(tagsPath)) {
    console.error(`错误：tags 文件夹不存在: ${tagsPath}`);
    process.exit(1);
}

if (!fs.existsSync(wordPath)) {
    console.error(`错误：word 文件夹不存在: ${wordPath}`);
    process.exit(1);
}
    ...
    fs.readdirSync(folderPath).forEach((file) => {
        if (file.endsWith('.md')) {
            ...
            if (newContent !== content) {
                ...
                console.log(`在文件 ${file} 中删除了标题为 "${targetTitle}" 的代码块`);
            }
        }
    });
};
...
const deleteFolder = (folderPath) => {
    if (fs.existsSync(folderPath)) {
        ...
        console.log(`删除了文件夹 ${folderPath} 及其子文件`);
    } else {
...
```bash
node AutoDeleteDocs.js 代码块标题 文件夹名称
```

> [!WARNING]
>
> **如果只传入一个参数，那么两个变量的值将一致**
